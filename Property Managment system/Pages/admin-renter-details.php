...
if ($id <= 0) {
    header('Location: admin-renters-view.php');
    exit;
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
        ...
        if ($confirmText !== 'DELETE') {
            ...
            $del = $pdo->prepare('DELETE FROM renter WHERE RenterID = ?');
            $del->execute([$id]);
            header('Location: admin-renters-view.php?deleted=1');
            exit;
        }
    }

    $stmt = $pdo->prepare('SELECT RenterID, Firstname, Lastname, email, phone FROM renter WHERE RenterID = ? LIMIT 1');
    $stmt->execute([$id]);
    $user = $stmt->fetch();
    ...
    $error = 'Unable to load this renter right now.';
    $user = [
        'RenterID' => $id,
        'Firstname' => '',
        'Lastname' => '',
...
<body>
        ...
        <main>
            <div class="wrap">
                <div class="top-row">
                    ...
                    <a class="back-link" href="admin-renters-view.php">← Back to Renters</a>
                </div>
                ...
                <div class="panel">
                    ...
                    <form>
                        ...
                        <div class="field">
                            <label>Role</label>
                            <input type="text" value="Renter" readonly>
                        </div>

                        <div class="actions">
                            <a class="btn btn-primary" href="admin-renter-edit.php?id=<?php echo (int)$user['RenterID']; ?>">Edit</a>
                            <button class="btn btn-danger" id="deleteBtn" type="button">Delete</button>
                        </div>
                    </form>

                    <form id="deleteForm" action="admin-renter-details.php?id=<?php echo (int)$id; ?>" method="POST" style="display:none;">
    ...
    <script>
        document.getElementById("deleteBtn").addEventListener("click", () => {
            const ok = confirm("Delete this renter?");
            if (!ok) {
                return;
            }

            const typed = prompt('Type DELETE to confirm permanently removing this renter.');
            if (typed !== 'DELETE') {
