<?php
// admin-lease-edit.php — Admin edits an existing lease record.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$renter_id) $errors[] = 'Please select a renter.';
    if (!$unit_id)   $errors[] = 'Please select a unit.';
    if (!$emp_id)    $errors[] = 'Please select a managing employee.';
    if (!is_numeric($price) || (float)$price <= 0)
        $errors[] = 'Price must be a positive number.';
    if (!is_numeric($period) || (int)$period <= 0)
        $errors[] = 'Period must be a positive number of months.';

    if (empty($errors)) {
        $pdo->prepare('
            UPDATE lease
            SET RenterID = ?, UnitID = ?, EmpID = ?, Price = ?, period = ?
            WHERE LeaseID = ?
        ')->execute([(int)$renter_id, (int)$unit_id, (int)$emp_id, (float)$price, (int)$period, $id]);
        header('Location: admin-lease-details.php?id=' . $id . '&updated=1');
        exit;
    }

    // Keep POST values on error
    $v = compact('renter_id', 'unit_id', 'emp_id', 'price', 'period');
} else {
    // Load current lease values
    $stmt = $pdo->prepare('SELECT * FROM lease WHERE LeaseID = ? LIMIT 1');
    $stmt->execute([$id]);
    $lease = $stmt->fetch();
    if (!$lease) {
        header('Location: admin-leases-view.php');
...
<head>
    ...
    <title>Cowboy Properties | Edit Lease #<?php echo $id; ?></title>
    <style>
...
<div class="page">
    ...
    <main>
        <div class="wrap">
            <div class="top-row">
                <h1>Edit Lease #<?php echo $id; ?></h1>
                <a class="back-link" href="admin-lease-details.php?id=<?php echo $id; ?>">← Back to Details</a>
            </div>
            ...
            <div class="panel">
                ...
                <form method="POST" novalidate>
                        ...
                        <select id="renter_id" name="renter_id" required>
                            ...
                            <?php foreach ($renters as $r): ?>
                                <option value="<?php echo (int)$r['RenterID']; ?>"
                                    <?php echo $v['renter_id'] == $r['RenterID'] ? 'selected' : ''; ?>>
                        ...
                        <select id="unit_id" name="unit_id" required>
                            ...
                            <?php foreach ($units as $u): ?>
                                <option value="<?php echo (int)$u['UnitID']; ?>"
                                    <?php echo $v['unit_id'] == $u['UnitID'] ? 'selected' : ''; ?>>
                        ...
                        <select id="emp_id" name="emp_id" required>
                            ...
                            <?php foreach ($employees as $e): ?>
                                <option value="<?php echo (int)$e['EmpID']; ?>"
                                    <?php echo $v['emp_id'] == $e['EmpID'] ? 'selected' : ''; ?>>
                    ...
                    <div class="row-2">
                        <div class="field">
                            ...
                            <input id="price" name="price" type="number" step="0.01" min="0"
                                   value="<?php echo htmlspecialchars($v['price']); ?>" required />
                        </div>
                        <div class="field">
                            <label for="period">Period (months) *</label>
                            <input id="period" name="period" type="number" min="1"
                                   value="<?php echo htmlspecialchars($v['period']); ?>" required />
                    ...
                    <div class="actions">
                        ...
                        <a class="btn btn-outline" href="admin-lease-details.php?id=<?php echo $id; ?>">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
